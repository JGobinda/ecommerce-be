...
class OrderSerializer(DynamicFieldsModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if view and view.action.lower() in ['payment_verification']:
            total = \
                Order.objects.filter(user=request.user, status=PENDING, is_paid=False).aggregate(
    ...
    def get_total_price(self, obj):
        request = self.context.get('request')
        return \
            Order.objects.filter(user=request.user, status=PENDING, is_paid=False).aggregate(total=Sum('total_price'))[
                'total']
